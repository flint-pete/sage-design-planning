...
Companion: `Infra-problems-to-fix.md` (the raw issue list this
...
## 2. Node identity & location as first-class calls (Infra #1) — IMPLEMENTED

> **This section has shipped and moved.** The node-identity work is built, tested,
> and packaged as its own repo — **`pywaggle2-nodeinfo`** (the reader) + its producer
> **`wes-nodeinfo-injection`** (the WES side). The full, current design lives in
> `pywaggle2-nodeinfo/DESIGN.md`; the CI handoff is `pywaggle2-nodeinfo/HANDOFF.md`.
> This stub keeps just enough context for §1/§3 to reference; do NOT edit the design
> here — edit it in the repo (this file is only the roadmap for the still-unbuilt
> §1/§3 work).

**In one paragraph:** a running plugin cannot learn its own VSN or GPS today (pywaggle
0.56 has no identity/location API; the manifest is a node-HOST path not mounted into
pods; identity is only attached downstream by Beehive routing). The fix is a
first-class accessor — `Plugin.get_node_info() -> NodeInfo(vsn, node_id, lat, lon,
mobility, ...)` — backed by WES injecting five `WAGGLE_NODE_*` env vars into every
pod, with the reader normalizing every sentinel to `None`/`"unknown"` (never
fabricate a coordinate). `mobility` is a tri-state (`static`/`mobile`/`unknown`) that
governs whether re-polling live GPS is ever worthwhile. A live-GPS Tier-2 path
wrapping `wes-gps-server` gpsd is designed but not yet built; it is complementary
(for mobile nodes) and reads this same env for the static tier.

**Status:** env/reader path verified end-to-end on H00F (inject → read → geotag →
upload → Sage data API). Remaining for CI: merge the two `wes-nodeinfo-injection`
patches upstream and add the `mobility` manifest field. See the repo for everything.

Everything below in §1 (acquisition) and §3 (other library enhancements) is the part
of pywaggle2 that is **still design-only** — that is what this document now tracks.

---

<details>
<summary>Original §2 detail (superseded by pywaggle2-nodeinfo/DESIGN.md — kept folded for history)</summary>
...
5. **Env vars are the primary source; mounted manifest is the fallback.** Prefer
   ...
   **[REFINED by §2.4, 2026-07-09 — after reading WES source + inspecting live
   nodes.]** The env-primary half stands, but the fallback is NOT the raw manifest:
   the raw `node-manifest-v2.json` must NEVER be mounted into plugin pods (it carries
   sensor URIs/creds, DevEUIs, serials, modem, street address — see §2.4.2). The
   file fallback is a WES-curated, whitelisted `node-info.json` (§2.4.3). Precedence
   unchanged from the library's view: explicit arg > env scalars > mounted curated
   file.
...
### 2.3 The WES side — IMPLEMENTED (`wes-nodeinfo-injection` v1.0.0)

> **STATUS (2026-07-13): built and verified end-to-end on real hardware (H00F).**
> The WES side described here is no longer a proposal — it ships as
> `wes-nodeinfo-injection` v1.0.0 (two upstream patches: `update-stack.sh` adds
> GPS+mobility to the `wes-identity` ConfigMap; `edge-scheduler` projects it into
> every plugin pod via `EnvFrom`). The reader below (`node_info_env.py`) is the
> pywaggle2-side consumer and is exercised by 7 end-to-end tests + a live H00F
> geotagged-upload round-trip. What remains for the CI team is merging the two
> patches upstream and adding the `mobility` manifest field (see that repo's
> HANDOFF.md).

For the API to have data to return (and for non-Python plugins), WES injects node
identity into every pod as env vars (`WAGGLE_NODE_VSN`, `WAGGLE_NODE_ID`,
`WAGGLE_NODE_GPS_LAT`, `WAGGLE_NODE_GPS_LON`, **`WAGGLE_NODE_MOBILITY`**).
pywaggle2's accessor reads them and normalizes per §2.2.3: when a field is unset,
sentinel-valued, or null it resolves to `None` (mobility → `"unknown"`). The wire
contract (what WES emits, what the reader expects):
...
(This is the env channel; a richer, still-whitelisted `node-info.json` file channel
is designed but deferred — env covers the five identity scalars. If the CI team's
runtime "GPS call" is itself a gpsd wrapper, §2.2.1/§6.3, pywaggle2 wraps the same
source rather than double-wrapping.)
...
### 2.4 WES delivery mechanism — grounded in the source (2026-07-09)

This section resolves the open question "does EVERY manifest variable get passed to
the plugin via env, or only a handful — and should we go env-vars or a filesystem/
manifest mount?" by reading the actual WES source (`waggle-sensor/waggle-edge-stack`,
shallow clone) and inspecting two live nodes (H00F camera node; W096 LoRaWAN node).
It SUPERSEDES the earlier lean in §2.2.4 #5 ("env primary, mounted RAW manifest as
fallback"): the raw manifest must NOT be mounted into plugin pods.

#### 2.4.1 What WES does today (from source)

WES already uses BOTH channels, split deliberately — small scalars via env,
structured config via a file. Neither is the raw manifest:

- **Env, a curated handful.** `kubernetes/update-stack.sh::update_wes()` generates
  `configs/wes-identity.env` containing exactly:
  `WAGGLE_NODE_ID` and `WAGGLE_NODE_VSN` (derived from `node_id`/`node_vsn`, i.e.
  `/etc/waggle/{node-id,vsn}`). Kustomize (`kustomization.yaml.keep`,
  `configMapGenerator`) turns that into the **`wes-identity` ConfigMap**, consumed
  by many WES components via `envFrom: configMapRef: name: wes-identity`
  (upload-agent, ssh-key updater, chirpstack, scheduler, …). So there is ALREADY a
  node-identity env ConfigMap — it simply carries only VSN + node_id today, NOT
  gps/mobility.
- **File, for structured config.** Plugin pods get `data-config.json` from a
  **ConfigMap `waggle-data-config`**, created by
  `update-stack.sh::update_data_config()`
  (`kubectl create configmap waggle-data-config --from-file=data-config.json=...`)
  and mounted read-only via **`subPath`** at `/run/waggle/data-config.json`
  (canonical example: `kubernetes/wes-dev-notebook.yaml`, which carries the
  `role: plugin` labels — it is the plugin-pod template to mirror).
- **The raw manifest is host-only, by pattern.** `node-manifest-v2.json` is synced
  from `auth.sagecontinuum.org/manifests/<vsn>/` to `/etc/waggle/` and mounted into
  a few PRIVILEGED WES SYSTEM pods via **hostPath** (device-labeler,
  camera-provisioner, chirpstack-tracker) — never into a plugin pod. Plugin pods
  today get NO manifest at all.

Answer to the question: **only a handful (VSN, node_id) go via env today, and
structured data already goes via a mounted curated file — never the raw manifest.**
WES has thus ALREADY chosen "scalars→env, structured→file, raw manifest stays on
the host." The right design extends that existing split, it does not invent a new one.

#### 2.4.2 The raw manifest is NOT safe to expose to plugins (verified)

Mounting `node-manifest-v2.json` wholesale would over-share. Inspecting the real
manifests:

- **W096 (LoRaWAN):** `lorawanconnections[]` has **9 entries**, each with a
  `lorawandevice.deveui` (16-hex DevEUI device identifier) plus per-device
  operational telemetry (`last_seen_at`, `margin` RF link margin, `battery_level`,
  `expected_uplink_interval_sec`, `created_at`) — cross-tenant data about every
  other instrument on the node. (Good news, verified: the manifest stores DevEUIs
  but NOT the OTAA join secrets — AppKey/NwkKey/AppEUI live in the LoRaWAN network
  server, not here — so it is not a key leak; it is still identity/inventory +
  cross-tenant telemetry a plugin has no need for.)
- **H00F (camera):** `sensors[].uri` = `http://<internal-IP>/stw-cgi/...` — exposes
  internal network topology, and on nodes still using the credentialed-URI pattern
  (Infra #10, `rtsp://user:pass@host`) it would leak **camera credentials** to
  every plugin. `computes[].serial_no` = the compute module's hardware serial.
- **Both nodes:** `address` = a precise street address ("1020 S Union Ave,
  Chicago…" / the Argonne address) — beyond what `gps_lat/lon` already provides,
  and PII/site-sensitivity-adjacent.
- The schema also holds `modem` (null on both sampled nodes, but on cellular nodes
  can carry APN/SIM/IMEI) — treat as sensitive-by-default until a populated example
  is seen.

**Principle: whitelist, not blacklist.** `deveui` is the proof — its name contains
no "key"/"secret"/"password", so a naive strip-what-looks-secret blacklist would
pass it straight through. Only an explicit allow-list ("plugins get exactly these
fields") is safe against fields whose names don't advertise sensitivity AND against
new sensitive fields added upstream later.

#### 2.4.3 Recommended design — a curated projection, mirroring `data-config.json`

Do NOT mount the raw manifest. Instead WES publishes a **curated node-info view**,
generated from the manifest by whitelist, delivered on BOTH channels (mirroring the
env+file split WES already uses). pywaggle2's resolver already does per-field
precedence (explicit arg > runtime/env > mounted file), so supporting both costs
the library nothing.

1. **Env scalars — extend the EXISTING `wes-identity` ConfigMap.** Add
   `WAGGLE_NODE_GPS_LAT`, `WAGGLE_NODE_GPS_LON`, `WAGGLE_NODE_MOBILITY` alongside
   the current `WAGGLE_NODE_VSN`/`WAGGLE_NODE_ID` (env is inherently a whitelist —
   CI explicitly chooses each var, so it is safe by construction). Concretely: a few
   lines in `update-stack.sh` writing the extra vars into `wes-identity.env`, then
   ensure plugin pods get `envFrom: wes-identity` (system pods already do; the
   plugin-pod path is injected by the scheduler binary — see 2.4.4). This is the
   MINIMAL, lowest-risk ask.
2. **Curated file — a NEW `waggle-node-info` ConfigMap**, generated from the
   manifest by whitelist and mounted read-only via `subPath` at
   `/run/waggle/node-info.json`, exactly like `waggle-data-config`/`data-config.json`.
   Whitelist contents: `vsn`, `node_id` (name), `gps_lat`, `gps_lon`, `mobility`,
   `tags`, `phase`, `project`, and a SANITIZED `sensors` list (name +
   hardware model/capabilities only — NO `uri`, NO `serial_no`). Withheld entirely:
   `sensors[].uri`, all `serial_no`, `lorawanconnections[]` (at most a count/
   capability flag), `modem`, and the precise `address`.

The pitch to CI is therefore not "mount your manifest into pods" (which a
security-conscious reviewer should reject) but "add a curated `node-info.json`
ConfigMap next to the `data-config.json` one you already generate, and add 3 fields
to the `wes-identity` ConfigMap you already generate." Both are minimal diffs to
mechanisms that already exist.

#### 2.4.4 Where the plugin-pod spec is actually built (concrete diff, from source)

The standard mounts/env on EVERY scheduled plugin pod are injected by the
**edge-scheduler** binary (`waggle/edge-scheduler:0.28.0`, entrypoint
`nodescheduler`), NOT by the YAMLs in `waggle-edge-stack`. Source-confirmed
(2026-07-09): `edge-scheduler/pkg/nodescheduler/resourcemanager.go`, the pod-template
builder (~L589 env, ~L677 volumes, ~L718 volumeMounts). `wes-dev-notebook.yaml` is a
faithful hand-written mirror of what this emits.

Key finding — the GPS wiring already exists. That builder ALREADY injects, into
every plugin pod (L589–L649):
- `WAGGLE_PLUGIN_HOST/PORT/USERNAME/PASSWORD`, `WAGGLE_APP_ID` (fieldRef uid),
  `WAGGLE_SCOREBOARD`, `WAGGLE_GPS_SERVER = wes-gps-server.default.svc.cluster.local`,
  `ROS_*`, `HOST` (fieldRef `spec.nodeName`).
So **`WAGGLE_GPS_SERVER` is already present** — pywaggle2's Tier-2 `GPS()` default
(§2.2.1) is real with no WES change. And `HOST` already carries the node's k8s node
name. But there is NO `WAGGLE_NODE_VSN`, NO `WAGGLE_NODE_ID`, NO gps/mobility in the
plugin env, and NO `envFrom` — the plugin pod does NOT get the `wes-identity`
ConfigMap the WES SYSTEM pods get. That is the concrete gap.

The two concrete diffs to make node-info reach ALL plugins:

1. **`edge-scheduler` — `resourcemanager.go` pod-template builder:**
   - Add an `EnvFrom` on the plugin container referencing the existing
     `wes-identity` ConfigMap (Optional=true so a node mid-upgrade without the CM
     doesn't fail to schedule):
     `EnvFrom: []apiv1.EnvFromSource{{ConfigMapRef: &apiv1.ConfigMapEnvSource{
     LocalObjectReference: {Name: "wes-identity"}, Optional: booltoPtr(true)}}}`.
     This delivers `WAGGLE_NODE_VSN/ID` (+ the new gps/mobility scalars once #2 in
     §2.4.3 adds them to `wes-identity.env`) to every plugin, reusing the same
     ConfigMap the system pods already consume.
   - Add a `waggle-node-info` volume + `subPath` mount at
     `/run/waggle/node-info.json`, appended to the `volumes` (~L677) and
     `volumeMounts` (~L718) slices — structurally identical to the existing
     `waggle-data-config` entries three lines above. Make it Optional so pods still
     schedule before the CM exists.
2. **`waggle-edge-stack` — `update-stack.sh` (config generation):**
   - `update_wes()`: append `WAGGLE_NODE_GPS_LAT/LON` and `WAGGLE_NODE_MOBILITY`
     (read from the manifest via `jq`) to the `configs/wes-identity.env` heredoc
     (currently only VSN+ID, L200–L203).
   - Add an `update_node_info()` (mirror of `update_data_config()`, L115) that
     produces a whitelisted `node-info.json` from `node-manifest-v2.json` via `jq`
     and `kubectl create configmap waggle-node-info --from-file=node-info.json=...`.
     The `jq` filter is the machine-readable form of the §2.4.3 whitelist
     (emit vsn/name/gps/mobility/tags/phase/project + sanitized sensors; drop
     uri/serial_no/lorawanconnections/modem/address).

This is a small, additive change on both sides, reusing mechanisms that already
exist (`wes-identity` ConfigMap; the `data-config.json` generate-and-subPath-mount
pattern). No new privileged access, no hostPath into plugins, no raw manifest
exposure. It is the concrete, reviewable form of the ask for the CI team.
...
time-window selectors are DEFERRED there (this doc only pins the API shape).
FULL DESIGN: the `wes-local-cache-manager` repo (DESIGN-AND-PURPOSE.md = the
two-layer model + the WES manager pod + why hostPath needs a purpose-built quota;
HANDOFF.md = the mount mechanics). The Layer-1 library primitive lives as
`image-sampler2/cache.py`, the working reference to hoist into pywaggle2.
...
### 6.1 GPS testing — what's confirmed vs. still blocked
...
- **CONFIRMED on real hardware — the Tier-2 `gps_stream()` / `GPS().watch()`
  mechanism.** A live TPV stream was read from `wes-gps-server` gpsd on a static
  pole node (W09E) — 3D fix, 16 sats, and over successive fixes the position
  jittered at the 1e-7–1e-6° level (≈cm–2 m) with zero physical motion. That
  demonstrates the two-axes model on real hardware: a STATIC node emits a live,
  time-varying fix, and that jitter is receiver noise, not travel. gpsd holds the
  device exclusively, so a socket wrapper (not raw device reads) is the correct
  design. Crucially W09E's manifest `gps_lat/lon` is **null (unsurveyed)** while
  gpsd serves a good drifting fix — exactly the case the "static → manifest
  authoritative, ignore live jitter" rule (§2.2.1) is built for, and the "location
  genuinely unknown → return None, omit EXIF GPS" invariant. The full Tier-2
  contract (connect → VERSION → `?WATCH` → TPV/SKY stream → close) was exercised
  end-to-end, so the streaming path is no longer unexercised.
- **STILL BLOCKED — deployment-mobility SEMANTICS.** What genuinely needs real
  ...
  (real travel, not jitter), plus a non-`static` `mobility` manifest field. That
  validates the authoritative-source switch (manifest vs. live) and the `max_age`
  re-poll cadence under motion. All current fleet nodes are static deployments, so
  this waits on mobile nodes shipping — it does not block v1.

---

## 7. Status & next step

DRAFT for discussion. **§2 (node identity) has shipped** — built, tested, and
verified end-to-end on H00F, packaged as `pywaggle2-nodeinfo` + `wes-nodeinfo-
injection`; see those repos, not this doc, for its current state. What this document
now tracks is the **still-unbuilt** work: §1 (the acquisition & metadata redesign)
and §3 (other library enhancements). The GPS source for the future live-GPS tier is
already empirically confirmed (a live TPV stream read from `wes-gps-server` gpsd on a
static node, §6.1), validating the two-axes model and the "static → manifest
authoritative" rule on real hardware.

§1 (acquisition ladder) and §3 (library enhancements) are the natural content of a
fuller upstream RFC to `waggle-sensor/pywaggle`, anchored by image-sampler2 as the
reference implementation.

### 7.1 Remaining work

1. **Node identity — DONE for the env/static tier.** Shipped as `pywaggle2-nodeinfo`
   v0.1.0 + `wes-nodeinfo-injection` v1.0.0. CI-side remainder (merge the two patches
   upstream, add the `mobility` manifest field) is tracked in those repos' HANDOFFs,
   not here. The future live-GPS Tier-2 wrapper (§2.2.2 `read_averaged()` etc.) is the
   only node-info piece still in this doc's scope.
...
3. **Decide the two small open API knobs**: the `read_averaged()` GPS convenience
   (jitter smoothing, optional) and the snapshot-URL/MJPEG advertisement home for
   named cameras (§6.4 — extend `data-config.json` vs. companion file).
4. **Then: promote DRAFT → RFC** for the §1/§3 work — the fuller upstream RFC to
   `waggle-sensor/pywaggle` anchored by image-sampler2.

What's left is a camera probe (item 2), two minor API decisions (item 3), and the
optional live-GPS tier — not further design of the shipped node-info core.
