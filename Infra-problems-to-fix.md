...
`plugin-improvements.md`. Cross-cutting needs (a platform primitive
...
- **pywaggle:** add a first-class location/identity accessor mirroring
  ...
  (See the fuller pywaggle redesign: `pywaggle2-design.md` §2, which
...
## 2. [BUG][P1] ✅ RESOLVED — ECR/Jenkins buildkit fails on EVERY `RUN` step (`/proc/acpi` runc error)
...
**RESOLVED:** 2026-07-10 — CI team fixed the Thor/buildkit runc issue. VERIFIED: the
ECR "Register and Build" pipeline successfully built `birdnet-species` v0.2.1
(`flint-pete/birdnet` @ c8e35a0, arm64, python:3.12-slim) — a Dockerfile with three
`RUN` steps (apt-get, pip install, build-time model pre-download), i.e. exactly the
path that previously died at container init. The historical detail below is retained
as the record of the bug + how it was proven; do NOT re-file.
...
framing was wrong. See the `wes-local-cache-manager` repo (HANDOFF.md, "How a plugin gets the /local-cache mount").
...
`--snapshot-url "http://.../api.cgi?...&user=sage&password=<cleartext>&..."`.
